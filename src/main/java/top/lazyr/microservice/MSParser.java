...
public class MSParser {
    ...
    private void initApiOfSvc(Service service, List<CtClass> ctClasses) {
        for (CtClass ctClass : ctClasses) {
            if (TypeManager.isAPI(ctClass)) {
                String prefix = TypeManager.extractApiPrefix(ctClass);
                CtMethod[] ctMethods = ctClass.getMethods();
                for (CtMethod ctMethod : ctMethods) {
                    if (TypeManager.isApiFunc(ctMethod)) {
                        Set<Api> apis = TypeManager.extractApiFromApiFunc(ctMethod, prefix);
                        service.addApi(apis, TypeManager.extractFileName(ctClass.getName()));
                    }
                }
            }
        }
    ...
    private void initFeignInfo(String msPath) {
        ...
        System.out.println(ctClasses.size());
        for (CtClass ctClass : ctClasses) {
            if (TypeManager.isFeign(ctClass)) {
                feignFileName2Feign.put(TypeManager.extractFileName(ctClass.getName()), buildFeign(ctClass));
            }
        }
...
//        }
    ...
    private Feign buildFeign(CtClass ctClass) {
        ...
        String svcName = TypeManager.extractSvcName(ctClass);
        Feign feign = new Feign(feignFileName, svcName);

        // 一定以 "/" 开头
        String prefix = TypeManager.extractFeignPrefix(ctClass);
...
//                String longFuncName = ctClass.getName() + "." + TypeManager.extractMethodName(ctMethod.getLongName());
    ...
    private class SvcCallBuilder extends ExprEditor {
        ...
        public SvcCallBuilder(String inClassName, Service inSvc) {
            this.inFileName = TypeManager.extractFileName(inClassName);
            this.inSvc = inSvc;
        }
        ...
        public void buildCall(String outClassName, String longFuncName) {
            String feignName = TypeManager.extractFileName(outClassName);
